...
export const cartSlice = createSlice({
  ...
  reducers: {
    addToCart(
      ...
      if (
        state.cart.some(
          (cartedProduct) => cartedProduct.title === addedProduct.title
        )
      ) {
      ...
      state.cart.forEach((cartedProduct) => {
        if (cartedProduct.title === removedProduct.title) {
          ...
          const index = state.cart.findIndex(
            (cartedProduct) => cartedProduct.title === removedProduct.title
          );
      ...
      state.cart.forEach((cartedProduct) => {
        if (cartedProduct.title === action.payload.product.title) {
          if (cartedProduct.purchasedQuant > 1) {
            ...
            const index = state.cart.findIndex(
              (cartedProduct) =>
                cartedProduct.title === action.payload.product.title
            );
            state.cart.splice(index, 1);
          }
        }
